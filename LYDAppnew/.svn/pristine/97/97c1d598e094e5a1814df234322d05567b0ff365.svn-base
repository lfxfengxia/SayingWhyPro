...
//手机号码验证
+ (BOOL) justMobile:(NSString *)mobileNum
{
...
//是否包含汉字
+ (BOOL) containChineseWord:(NSString *)contentStr {
    for (int i = 0; i < [contentStr length]; i++) {
        int charValue = [contentStr characterAtIndex:i];
        if(charValue > 0x4e00 && charValue < 0x9fff) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)justCorrentPassword:(NSString *)password {
