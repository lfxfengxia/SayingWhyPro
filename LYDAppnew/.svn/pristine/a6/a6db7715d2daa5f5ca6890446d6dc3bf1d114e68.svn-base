...
@implementation XYHomeLDBCell

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"XYHomeLDBCell";
    XYHomeLDBCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[XYHomeLDBCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
        [cell createUI];
    }
    return cell;
}
...
//    [self.contentView addSubview:_smallImageView];
    ...
    image.image=[UIImage imageNamed:@"钱-4"];
...
-(void)QiangGouClick:(UIButton *)btn
{
    ...
    [self.delegate didClickButtonXYHomeLDBCell:btn];
