...
@interface DSYCouponUnUseController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation DSYCouponUnUseController
...
- (NSDictionary *)getMyParaWithIndex:(NSInteger)pageIndex {
    ...
    NSDictionary *secretDict = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"status":@1};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"status":@1, @"sign":sign};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DSYCouponSecondCell *cell = [DSYCouponSecondCell cellForTableView:tableView];
