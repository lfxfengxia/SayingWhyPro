//
//  DSYSanbidDetailViewController.m
//  LYDApp
//
...
@interface DSYSanbidDetailViewController ()
<XYErrorHudDelegate, UITableViewDelegate, UITableViewDataSource>
...
@implementation DSYSanbidDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.titleNavigationBarLabel.text = self.sanbid.title;
    ...
    // 新手专项标不可使用优惠券
    [self loadMyCoupon];
//    self.investLabel.text = [NSString stringWithFormat:@"￥%.2f", [self.amount integerValue] * [self.sanbid.minAmount floatValue] * 1.0];
//    CGFloat investAmout = [[self.investLabel.text substringFromIndex:1] floatValue];
//    NSInteger months = [self.sanbid.periods integerValue];
//    CGFloat rate = [self.sanbid.apr floatValue] / 100 + 0;
//    self.preBenifitLabel.text = [NSString stringWithFormat:@"￥%.2f", investAmout * months * rate / 12];
    // 初始是没有优惠券
    [self setupMyUI];
    
    NSLog(@"%@ ==== %@", self.preBenifitLabel.text, [NSString stringWithFormat:@"￥%.2f", [self.investLabel.text floatValue] * [self.sanbid.periods integerValue] * [self.sanbid.apr floatValue] / (12 * 100)]);
}
...
- (void)setupMyUI {
    ...
    if ([DSYAccount sharedDSYAccount].canInvestPlanNew == 0) {
    ...
    CGFloat apr = 0;
    // 总投资金额
    CGFloat allInvestAmount =  [self.amount integerValue] * [self.sanbid.minAmount floatValue] * 1.0;
    // 投资金额的显示
    ...
    // 期数
    NSInteger periods = [self.sanbid.periods integerValue];
    CGFloat actual = [self.sanbid.apr floatValue] + apr;
    CGFloat rate = actual / 100  / 12;
    // 如果是日
    if ([self.sanbid.periodUnit integerValue] == 1) {
        // 日
        rate = actual / 365 / 100;
    } else if ([self.sanbid.periodUnit integerValue] == -1) {
        // 年
        rate = actual / 100;
    } else if ([self.sanbid.periodUnit integerValue] == 2) {
        // 周
...
- (void)loadMyCoupon {
    ...
    NSDictionary *secretDict = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"planId":self.sanbid.bidId};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"planId":self.sanbid.bidId, @"sign":sign};
...
- (void)createUI
{
    ...
    // 预期年化利率的显示
    NSMutableAttributedString *rateAttr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%.1f%%",[self.sanbid.apr floatValue]]];
    ...
    if ([self.sanbid.periodUnit integerValue] == 0) {
        period = [NSMutableString stringWithFormat:@"%@月",self.sanbid.periods];
    } else if ([self.sanbid.periodUnit integerValue] == 1) {
        period = [NSMutableString stringWithFormat:@"%@日",self.sanbid.periods];
    } else if ([self.sanbid.periodUnit integerValue] == -1) {
        period = [NSMutableString stringWithFormat:@"%@年",self.sanbid.periods];
    } else if ([self.sanbid.periodUnit integerValue] == 2) {
        period = [NSMutableString stringWithFormat:@"%@周",self.sanbid.periods];
    }
    ...
    self.moneyTextLabel.text = @"最高可投金额";
    [self.infoHeadView addSubview:self.moneyTextLabel];
    
    self.moneyLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.timeLabel.maxX + 10, self.moneyTextLabel.maxY + KHeight(12), self.infoHeadView.width - self.timeLabel.maxX - 40, KHeight(22))];
    self.moneyLabel.textColor = [UIColor blackColor];
    self.moneyLabel.font = [UIFont systemFontOfSize:KHeight(28)];
    self.moneyLabel.textAlignment = NSTextAlignmentCenter;
    self.moneyLabel.adjustsFontSizeToFitWidth = YES;
    self.moneyLabel.centerX = self.infoHeadView.width * 5 / 6;
    
    NSString *showMaxInvestAmount = [NSString stringWithFormat:@"%.0f元", self.sanbid.maxAmount];
    NSMutableAttributedString *moneyAttr = [[NSMutableAttributedString alloc] initWithString:showMaxInvestAmount];
    [moneyAttr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:KHeight(16)] range:NSMakeRange(showMaxInvestAmount.length - 1, 1)];
    [moneyAttr addAttribute:NSForegroundColorAttributeName value:TEXTBLACK range:NSMakeRange(showMaxInvestAmount.length - 1, 1)];
...
- (void)createPayUI
{
    ...
    self.discountLabel = [[UIButton alloc] initWithFrame:CGRectMake(self.amountTextLabel.x, line2.maxY + KHeight(20), kSCREENW - self.amountTextLabel.x - KWidth(28) - KWidth(40), KHeight(12))];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    couponRate = coupon.amount / 100;
    self.coupon = coupon;
    
    if (self.coupon.minInvestAmount > [self.amount integerValue] * [self.sanbid.minAmount integerValue]) {
        // 如果没有达到优惠券的最小投资金额
...
- (void)investCountFieldValuechanged:(UITextField *)textField {
    ...
    CGFloat investMoney = [self.amount integerValue] * [self.sanbid.minAmount floatValue];
    if (investMoney  >= 200000.0) {
        NSInteger maxCount =  (NSInteger)(200000 / ([self.sanbid.minAmount floatValue]));
...
{
    CGFloat investAllAmount = [self.amount integerValue] * [self.sanbid.minAmount floatValue];
    ...
    } else if (investAllAmount > [self.myBalance floatValue]) {
...
//        confirmVC.model = self.sanbid;
        // 生成一个相似的XYPlayModel
        XYPlanModel *model = [[XYPlanModel alloc] init];
        model.planId = self.sanbid.bidId;
        model.title = self.sanbid.title;
        model.apr = self.sanbid.apr;
        model.periodUnit = self.sanbid.periodUnit;
        model.periods = self.sanbid.periods;
        model.perAmount = self.sanbid.minAmount;
        model.repaymentType = @"2";
...
{
    [self discountLabelTapped:nil];
    XYPlanPlanDetailController *planDetailVC = [[XYPlanPlanDetailController alloc] init];
    planDetailVC.planId = self.sanbid.bidId;
