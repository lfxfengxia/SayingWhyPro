...
#import "DSYCouponFourthCell.h"

#define kDSYPageSize 15

@interface DSYCouponOverduedController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation DSYCouponOverduedController
...
- (NSDictionary *)getMyParaWithIndex:(NSInteger)pageIndex {
    ...
    NSDictionary *secretDict = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"status":@3};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"status":@3, @"sign":sign};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DSYCouponFourthCell *cell = [DSYCouponFourthCell cellForTableView:tableView];
