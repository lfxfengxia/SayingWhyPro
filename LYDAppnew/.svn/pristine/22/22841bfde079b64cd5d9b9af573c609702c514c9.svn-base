...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(change:) name:@"LOAD_DONE" object:nil];
    ...
    for (int i = 0; i < self.countImage; i++) {
        ...
        if (photoBrowserView.isLoadedImage) {
            ...
            [self.buttonSave setTitleColor:[UIColor grayColor]
                                  forState:UIControlStateNormal];
...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"LOAD_DONE" object:nil];
}

- (void)change:(NSNotification *)aNoti
{
    if ([[aNoti.userInfo valueForKey:@"Info"] isEqualToString:@"done"]) {
        [self.buttonSave setTitleColor:[UIColor whiteColor]
                              forState:UIControlStateNormal];
        [self.buttonSave setEnabled:YES];
    } else {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 4.预加载图片数据
//    NSInteger left = pageCurrent - 1;
//    NSInteger right = pageCurrent + 1;
//    left = left > 0 ? left : 0;
...
//    }
    ...
    [self setupImageOfImageViewForIndex:pageCurrent];
