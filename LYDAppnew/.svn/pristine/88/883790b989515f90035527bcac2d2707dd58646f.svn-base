...
@interface DSYDivergenceCompleteCell ()
...
@implementation DSYDivergenceCompleteCell
...
+ (DSYDivergenceCompleteCell *)cellForTableView:(UITableView *)tableView {
    static NSString *ID = @"divergenceCompleteCell";
    DSYDivergenceCompleteCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[DSYDivergenceCompleteCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)creatSubviews {
    ...
    self.buttonBGView.image = DSYImage(@"account_financing_cell_three.png");
    self.buttonBGView.userInteractionEnabled = YES;
    
    [self creatBtnWithTitles:@[@"服务协议", @"账单详情", @"评论"]];
}
