//
//  YiYueBiaoGouMaiViewController.m
//  LYDApp
//
...
#import "BuyMarkHeadViewYiYueBiao.h"
#import "FenShuInvestmentViewYiYueBiao.h"
#import "LYDPlanConfirmController.h"

@interface YiYueBiaoGouMaiViewController ()<UITableViewDelegate,UITableViewDataSource,XYErrorHudDelegate,ChongzhiDelegate>

@property(nonatomic,strong)BuyMarkHeadViewYiYueBiao *headView;
@property(nonatomic,strong)FenShuInvestmentViewYiYueBiao *investmentView;
...
@implementation YiYueBiaoGouMaiViewController
...
-(BuyMarkHeadViewYiYueBiao *)headView
{
    if (!_headView) {
        _headView=[[BuyMarkHeadViewYiYueBiao alloc]init];
    }
    return _headView;
}

-(FenShuInvestmentViewYiYueBiao *)investmentView
{
    if (!_investmentView) {
        _investmentView=[[FenShuInvestmentViewYiYueBiao alloc]init];
...
//    [self discountLabelTapped:nil];
    ...
    if (self.coupon.minInvestAmount > [self.investmentView.Field.text integerValue])
    {
    ...
    } else if ([self.investmentView.Field.text integerValue] > [self.myBalance integerValue]+youhuiquan) {
        
        XYErrorHud *errorHud = [[XYErrorHud alloc] initWithTitle:@"余额不足" andDoneBtnTitle:@"去充值" andDoneBtnHidden:NO];
        errorHud.delegate = self;
        [self.view.window addSubview:errorHud];
        
    } else if([self.investmentView.Field.text isEqualToString:@"0"] ||[self.investmentView.Field.text isEqualToString:@""])
    {
       [XYErrorHud showMessage:@"最少投100元！" toView:self.view];
    }else{
        
        LYDPlanConfirmController  *confirmVC = [[LYDPlanConfirmController alloc] init];
        confirmVC.model = self.model;
        confirmVC.sumMoney = self.investmentView.Field.text;//投资金额
...
-(void)requestCouponList{
    ...
    [LYDTool sendGetWithUrl:url parameters:para success:^(id data) {
        ...
        if (statusCode == 200) {
            ...
            NSString *showStr = [NSString stringWithFormat:@"可以使用%ld张优惠券", self.dataList.count];
            NSAttributedString *attr = [[NSAttributedString alloc] initWithString:showStr attributes:nil];
...
//        [self.discountLabel setAttributedTitle:attr forState:(UIControlStateNormal)];
    ...
    //自动赋值满足优惠券使用的最小金额
    self.investmentView.Field.text=[NSString stringWithFormat:@"%0.2f",self.coupon.minInvestAmount];;
