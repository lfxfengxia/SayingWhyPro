...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, ZHToobarHeight, kSCREENW, pickView.frame.size.height);
    NSLog(@"%f",pickView.frame.size.width);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.frame=CGRectMake(0, ZHToobarHeight, kSCREENW, datePicker.frame.size.height);
...
-(void)remove{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarCancleBtnClicked:)]) {
        [self.delegate toobarCancleBtnClicked:self];
    }
...
//                    _city=[dicValueDic allValues][0][0];
        ...
        _resultString=[NSString stringWithFormat:@"%@",prettyVersion];
    }
