...
@implementation BuyMarkHeadViewYiYueBiao
...
-(void)getAccount:(XYPlanModel *)model{
    self.investTime.text=[NSString stringWithFormat:@"%@个月",model.periods];
    self.eachAmount.text=[NSString stringWithFormat:@"%@",model.leftAmount];
    
    NSString *aprDiscount=[NSString stringWithFormat:@"%0.2f",[model.aprDiscount floatValue]];
    NSString *apr=[NSString stringWithFormat:@"%0.2f",[model.apr floatValue]-[model.aprDiscount floatValue]];
    NSMutableAttributedString *str=[[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"%@%%+%@%%",apr,aprDiscount]];


    [str addAttribute:NSFontAttributeName value: [UIFont systemFontOfSize:KHeight(34)] range:NSMakeRange(0, apr.length)];
    [str addAttribute:NSForegroundColorAttributeName value:RGB(255, 121, 1)  range:NSMakeRange(0, apr.length)];
    [str addAttribute:NSFontAttributeName value: [UIFont systemFontOfSize:KHeight(20)] range:NSMakeRange(apr.length, str.length-apr.length)];
    [str addAttribute:NSForegroundColorAttributeName value:RGB(255, 121, 1)  range:NSMakeRange(apr.length, str.length-apr.length)];
    self.earningspercent.attributedText=str;
