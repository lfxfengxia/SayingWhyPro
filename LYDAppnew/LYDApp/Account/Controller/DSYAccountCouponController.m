...
@interface DSYAccountCouponController ()<UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) DSYCouponAddController *addVC;          /**< 添加 */

@end

@implementation DSYAccountCouponController
...
- (void)setCurrentIndex:(NSInteger)currentIndex {
    ...
    [_topTabbar setSelectIndex:currentIndex];
    if (_currentIndex == 0) {
        [self unBindVC];
...
- (RBTitleView *)topTabbar {
    if (!_topTabbar) {
        ...
        __weak DSYAccountCouponController *weakSelf = self;
