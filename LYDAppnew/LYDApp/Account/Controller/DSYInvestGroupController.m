...
@interface DSYInvestGroupController ()<UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) NSMutableArray *dataList;
@property (nonatomic, strong) UITableView *contentTableView;
@property (nonatomic, assign) NSInteger pageIndex;
@end

@implementation DSYInvestGroupController
...
//    [self.dataList removeAllObjects];
    NSString *url = [NSString stringWithFormat:@"%@/user/invests/%ld/loans", APIPREFIX, self.financing.investId];
...
- (void)successDealWithData:(id)data {
    ...
    if (statusCode == 200) {
        ...
        if (self.pageIndex == 1) {
            ...
            self.dataList = [DSYInvestGroupModel baseModelByArr:data[@"investLoanList"]];
        } else {
            // 成功加载数据，当是加载更多的时候
            [self.dataList addObjectsFromArray:[DSYInvestGroupModel baseModelByArr:data[@"investLoanList"]]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DSYInvestGroupCell *cell = [DSYInvestGroupCell cellForTableView:tableView];
    
    cell.investGroup = self.dataList[indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    [self creatLabelsWithTitles:@[@"借款人", @"借款金额(元)", @"合同编号", @"投资金额"] forSuperView:labelView];
