//
//  DSYFinancingHoldingController.m
//  LYDApp
//
...
@interface DSYFinancingHoldingController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation DSYFinancingHoldingController
...
//    [self.dataList removeAllObjects];
    NSString *url = [NSString stringWithFormat:@"%@/user/investsVersionThree", APIPREFIX];
...
- (NSDictionary *)getMyParaWithIndex:(NSInteger)pageIndex {
    ...
    NSDictionary *secretDict = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"bidsType":[NSNumber numberWithInt:[self.bidType  intValue]], @"investStatus":@1, @"startDate":self.startDate, @"endDate":self.endDate};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize), @"bidsType":[NSNumber numberWithInt:[self.bidType intValue]], @"investStatus":@1, @"startDate":self.startDate, @"endDate":self.endDate, @"sign":sign};
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DSYFinancingModel *financing = self.dataList[indexPath.row];
    
    DSYFinancingDetailController *detailVC = [[DSYFinancingDetailController alloc] initWithFinancing:financing];
    
    if (financing.canTransfer == 0) {
        detailVC.titles = @[@"账单详情", @"服务协议", @"债权列表"];
    }
    
    detailVC.block = ^(NSInteger type) {
        [self.contentTableView.header beginRefreshing];
        DSYAccountFinancingViewController *parentVC = (DSYAccountFinancingViewController *)self.parentViewController;
        parentVC.completeRefresh = YES;
    };
