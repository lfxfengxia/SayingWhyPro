//
//  DSYDebtsCompleteViewController.m
//  LYDApp
//
...
@interface DSYDebtsCompleteViewController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) NSMutableArray *dataList;           /**< 保存数据的数据 */
@property (nonatomic, assign) NSInteger pageIndex;
@end

@implementation DSYDebtsCompleteViewController
...
- (NSDictionary *)getMyParaWithIndex:(NSInteger)pageIndex {
    ...
    NSDictionary *secretDict = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize),  @"startDate":self.startDate, @"endDate":self.endDate, @"status":@(4)};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN, @"pageIndex":@(pageIndex), @"pageSize":@(kDSYPageSize),  @"startDate":self.startDate, @"endDate":self.endDate, @"status":@(4), @"sign":sign};
