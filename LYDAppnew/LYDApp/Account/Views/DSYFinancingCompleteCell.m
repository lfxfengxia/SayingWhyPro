...
@interface DSYFinancingCompleteCell ()
...
@implementation DSYFinancingCompleteCell
+ (DSYFinancingCompleteCell *)financingCompleteCellForTableView:(UITableView *)tableView {
    static NSString *ID = @"financingCompleteCell";
    DSYFinancingCompleteCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[DSYFinancingCompleteCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:ID];
...
- (void)creatUI {
    ...
    [self creatBtnWithTitles:@[@"服务协议", @"账单详情", @"投资组合", @"评论"]];
