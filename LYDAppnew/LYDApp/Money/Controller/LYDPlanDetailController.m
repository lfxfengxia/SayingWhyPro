//
//  LYDPlanDetailController.m
//  LYDApp
//  
//  Created by fcl on 17/2/21.
...
#import "LYDPlanConfirmController.h"
#import "LDBDetailViewController.h"
#import "DSYHelpWebViewController.h"
#import "ChangJianWenTiViewController.h"
@interface LYDPlanDetailController ()<XYErrorHudDelegate, UITableViewDelegate, UITableViewDataSource>
...
//@property (nonatomic, strong) UIView      *minusButton;
//@property (nonatomic, strong) UIView      *plusButton;
...
@implementation LYDPlanDetailController
...
- (void)viewDidLoad {
    ...
    [self loadMyBalance];
    [self loadMyCoupon];//获取当前用户当前标的适用的优惠券
    [self setupMyUI];
}
...
- (void)setupMyUI {
    CGFloat apr = 0;
    // 总投资金额
    CGFloat allInvestAmount =[self.amountLabel.text floatValue];
    
    // 投资金额的显示
    //self.investLabel.text = [NSString stringWithFormat:@"￥%.2f", allInvestAmount];
    if ([self isBlankString:self.amountLabel.text]) {
        self.investLabel.text = @"￥0.00";
    }
    else
    {
        self.investLabel.text = [NSString  stringWithFormat:@"￥%@",self.amountLabel.text];
...
//    self.preBenifitLabel.text = [NSString stringWithFormat:@"￥%.2f", allInvestAmount *  periods * rate];
    ...
    CGFloat rateAdd11 = actualAdd / 100  / 12;
    // 如果是日
    if ([self.model.periodUnit integerValue] == 1) {
        // 日
...
- (void)createUI
{
    ...
    self.rateTextLabel = [[UILabel alloc] initWithFrame:CGRectMake(KWidth(58/2), KHeight(15), labelW, KHeight(11))];
    ...
    self.aprDiscountLabel = [[UILabel alloc] initWithFrame:CGRectMake(KWidth(58/2), self.rateLabel.maxY+ KHeight(12), labelW+5, KHeight(11))];
    ...
    [self.infoHeadView addSubview:self.aprDiscountLabel];
    
    NSString *Ratastr=[NSString stringWithFormat:@"+%.2f%%(贴息)",[self.model.aprDiscount floatValue]];
    NSUInteger loca1=(NSUInteger)(Ratastr.length-5);
    NSMutableAttributedString *str1=[[NSMutableAttributedString alloc]initWithString:Ratastr];
    [str1 addAttribute:NSForegroundColorAttributeName value:TEXTGARY range:NSMakeRange(loca1, 5)];
    //self.aprDiscountLabel.font = [UIFont systemFontOfSize:_font+57];
    self.aprDiscountLabel.attributedText = str1;
    
    
    // 预期年化利率的显示
    NSMutableAttributedString *rateAttr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%.2f%%",[self.model.apr floatValue]-[self.model.aprDiscount floatValue]]];
    ...
    self.timeTextLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.rateTextLabel.maxX + hMargin, KHeight(30), labelW, KHeight(11))];
...
//    self.timeLabel.attributedText = timeAttr;
    ...
    self.moneyTextLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.timeTextLabel.maxX + hMargin - 10, KHeight(30), labelW + 10, KHeight(11))];
    ...
    NSMutableAttributedString *moneyAttr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@元",self.model.leftAmount]];
...
//    [self.minusButton addTarget:self action:@selector(minusButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.amountLabel = [[UITextField alloc] initWithFrame:CGRectMake(self.minusButton.maxX, self.minusButton.y, KWidth(50)+200, KHeight(14))];
   ...
   // self.amountLabel.text=@"0";
    self.amountLabel.placeholder=@"0";
    self.amountLabel.userInteractionEnabled=YES;
    self.amountLabel.keyboardType=UIKeyboardTypeNumberPad;
    //investCountFieldValuechanged:
    [self.amountLabel addTarget:self action:@selector(investCountFieldValuechanged:) forControlEvents:(UIControlEventEditingChanged)];
//    [self.amountLabel addTarget:self action:@selector(showLabelTapAction:) forControlEvents:(UIControlEventTouchUpInside)];
//            UITapGestureRecognizer *tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showLabelTapAction:)];
//            [self.amountLabel addGestureRecognizer:tap1];
...
//    [self.plusButton addTarget:self action:@selector(plusButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    ...
    //self.preBenifitLabel.backgroundColor=[UIColor redColor];
...
//        _countBgLabel.textColor = [UIColor clearColor];
        ...
        _countBgLabel.backgroundColor = [UIColor redColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //自动赋值满足优惠券使用的最小金额
    self.amountLabel.text=[NSString stringWithFormat:@"%0.2f",self.coupon.minInvestAmount];;
...
//        //如果输入的金额为空字符串
//        self.investLabel.text = [NSString stringWithFormat:@"￥%.2f",0.00];
//
//    }
//    else
//    {
//         self.investLabel.text = [NSString stringWithFormat:@"￥%.2ld",[textField.text integerValue]];
//    }
//        self.investLabel.text = [NSString stringWithFormat:@"￥%.2f", [self.amount integerValue] * [self.model.perAmount floatValue] * 1.0];
    self.investLabel.text = [NSString stringWithFormat:@"￥%.2ld",[textField.text integerValue]];
...
//        } cancelHandler:^(UIAlertAction *action) {
//        }];
//    } else if ([self.amount integerValue] * [self.model.perAmount integerValue] > [self.myBalance integerValue]) {
//        
...
- (void)doneBtnClicked:(UIButton *)button
{
    ...
    if (self.coupon.minInvestAmount > [self.amountLabel.text integerValue])
    {
    ...
    if ([DSYAccount sharedDSYAccount].ipsAccount.length <= 0) {
    ...
    } else if ([self.amountLabel.text integerValue] > [self.myBalance integerValue]+youhuiquan) {
       ...
       LYDPlanConfirmController  *confirmVC = [[LYDPlanConfirmController alloc] init];
        confirmVC.model = self.model;
        confirmVC.sumMoney = self.amountLabel.text;//投资金额
