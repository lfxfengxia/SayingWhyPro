...
@interface XYPlanDetailController () <XYErrorHudDelegate, UITableViewDelegate, UITableViewDataSource>
...
@implementation XYPlanDetailController
...
- (void)loadMyCoupon {
    ...
    [LYDTool sendGetWithUrl:url parameters:para success:^(id data) {
        ...
        if (statusCode == 200) {
            self.dataList = [DSYCouponModel baseModelByArr:backData[@"couponModelList"]];
            NSString *str=[NSString stringWithFormat:@"%lu--%@",(unsigned long)self.dataList.count,[NSString stringWithFormat:@"您有%ld张优惠券可以使用", self.dataList.count]];
            NSLog(@"%@", str);
...
//    [self.infoHeadView addSubview:self.rateTextLabel];
    ...
    self.aprDiscountLabel.text = self.aprDiscountLabel.text = @"%(贴息)";
