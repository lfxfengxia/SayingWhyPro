...
@interface LYDPlanConfirmController ()
...
@implementation LYDPlanConfirmController
...
- (void)viewDidLoad {
    ...
    self.totalMoney = [self.sumMoney integerValue];
...
//    self.rateLabel.text = [NSString stringWithFormat:@"年化收益率：%.2f%%",[self.model.apr floatValue] + couponRate];
    self.rateLabel.text = [NSString stringWithFormat:@"年化收益率：%.2f%%+%.2f%%(贴息)",[self.model.apr floatValue]-[self.model.aprDiscount floatValue],[self.model.aprDiscount floatValue]];
...
//    [self.infoView addSubview:self.amountLabel];
    ...
    NSInteger payMoney = [self.sumMoney integerValue];
...
- (void)doPay {
    ...
    NSDictionary *secretDict = @{@"investAmount":[NSNumber numberWithInteger:self.totalMoney],@"bidType":self.model.bidType,@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN,@"planId":[NSString stringWithFormat:@"%@",self.model.planId],@"num":[NSString stringWithFormat:@"%@",@"1"],@"couponId":self.couponId};
    // 生成签名认证
    NSString *sign = [LYDTool createMD5SignWithDictionary:secretDict];
    NSDictionary *para = @{@"investAmount":[NSNumber numberWithInteger:self.totalMoney],@"bidType":self.model.bidType,@"appKey":APPKEY, @"timestamp":timestamp, @"deviceId":DEVICEID, @"token":TOKEN,@"planId":[NSString stringWithFormat:@"%@",self.model.planId],@"num":[NSString stringWithFormat:@"%@",@"1"],@"couponId":self.couponId,@"sign":sign};
...
//            XYErrorHud *errorHud = [[XYErrorHud alloc] initWithTitle:msg andDoneBtnTitle:nil andDoneBtnHidden:YES];
