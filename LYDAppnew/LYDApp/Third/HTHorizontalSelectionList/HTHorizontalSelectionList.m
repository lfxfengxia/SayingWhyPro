...
- (void)reloadData {
    ...
    if (totalButtons > 0) {
        ...
        NSLog(@"%f -- %f",selectedButton.frame.origin.x,selectedButton.frame.origin.y);
        ...
        switch (self.selectionIndicatorStyle) {
            case HTHorizontalSelectionIndicatorStyleBottomBar: {
                ...
                [self alignSelectionIndicatorWithButton:selectedButton];
                 NSLog(@"111 %f -- %f",selectedButton.frame.origin.x,selectedButton.frame.origin.y);
                break;
            }
    ...
    if (kSCREENW == 320) {
        [self.scrollView scrollRectToVisible:CGRectInset(CGRectMake((self.selectedButtonIndex * 57) + 10, 6, 42, 30), -kHTHorizontalSelectionListHorizontalMargin, 0) animated:YES];
    } else if (kSCREENW == 375) {
        [self.scrollView scrollRectToVisible:CGRectInset(CGRectMake((self.selectedButtonIndex * 57) + 10, 10, 42, 30), -kHTHorizontalSelectionListHorizontalMargin, 0) animated:YES];
    } else if (kSCREENW == 414) {
...
//    [self.scrollView scrollRectToVisible:CGRectInset([self.buttons[self.selectedButtonIndex] frame], -kHTHorizontalSelectionListHorizontalMargin, 0)
//                                         animated:YES];
//    NSLog(@"%f -- %f",[self.buttons[self.selectedButtonIndex] frame].origin.x,[self.buttons[self.selectedButtonIndex] frame].origin.y);
    NSLog(@"%ld",(long)self.selectedButtonIndex);
...
- (void)buttonWasTapped:(id)sender {
    ...
    if (index != NSNotFound) {
        ...
        [self.scrollView scrollRectToVisible:CGRectInset(tappedButton.frame, -kHTHorizontalSelectionListHorizontalMargin, 0)
                                    animated:YES];
        NSLog(@"tap  %f--%f--%f--%f",tappedButton.frame.origin.x,tappedButton.frame.origin.y,tappedButton.frame.size.width,tappedButton.frame.size.height);
