    ...
    if (self.imageView.image) {
        ...
        if (STFullWidthForLandScape){
            ...
            if (frame.size.width<=frame.size.height) {//竖屏时候
                ...
                if (imageFrame.size.width > imageFrame.size.height * 2) {
                    CGFloat ratio = frame.size.width/imageFrame.size.width;
                    imageFrame.size.height = imageFrame.size.height*ratio;
                    imageFrame.size.width = frame.size.width;
                } else {
    ...
    [self.imageView setImageWithURL:url
                      placeholderImage:placeholderImage
                              ...
                              progress:^(NSInteger receivedSize,
                                  ...
                                  if (error) {
                                      [strongSelf addSubview:weakSelf.buttonReload];
                                      NSDictionary *dict = [NSDictionary dictionaryWithObject:@"undone" forKey:@"Info"];
                                      [[NSNotificationCenter defaultCenter] postNotificationName:@"LOAD_DONE" object:nil userInfo:dict];
                                      return;
                                  } else {
                                      NSDictionary *dict = [NSDictionary dictionaryWithObject:@"done" forKey:@"Info"];
                                      [[NSNotificationCenter defaultCenter] postNotificationName:@"LOAD_DONE" object:nil userInfo:dict];
                                  }
